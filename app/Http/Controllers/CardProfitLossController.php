...
class CardProfitLossController extends Controller
{
    ...
    public function view_profit_loss(Request $request)
    {
        ...
        $query = DB::table('cardprofitloss')->orderBy('cardprofitloss.start_date', 'desc');
        ...
        if (request()->has('startdate')) {
            ...
            if ($start_date != "") {
                $query->whereDate('cardprofitloss.start_date', '>=', date('Y-m-d', strtotime($start_date)));
            }
            ...
            if (request()->has('enddate')) {
                ...
                if ($end_date == $start_date) {
                    $query->whereDate('cardprofitloss.start_date', '>=', date('Y-m-d', strtotime($end_date)));
                }
            }
        }
        ...
        if (request()->has("option")) {
            ...
            if ($option != "") {
                ...
                if ($option == 1) {
                    ...
                    $query->where('cardprofitloss.start_date', $date);
                    ...
                    $query->whereBetween('cardprofitloss.start_date', [$date, $today]);
        ...
        return view('cardprofit_loss.view_profit_loss', compact('profit', 'loss'));

    }

    public function view_daily_report(Request $request)
    {
        $amt = DB::table('carddaily_report')->select(DB::raw('SUM(net_amount)as net_amount'), DB::raw('SUM(total_received)as total_received'), DB::raw('SUM(total_withdraw)as total_withdraw'), DB::raw('SUM(cashfreeRper)as cashfreeRper'), DB::raw('SUM(cashfreeWper)as cashfreeWper'), DB::raw('SUM(profit)as profit'), DB::raw('SUM(loss)as loss'))->first();
        // dd($amt);
        return view('cardprofit_loss.daily_report', compact('amt'));
    }
    public function view_daily_report_dt(Request $request)
    {
        ...
        $query = DB::table('carddaily_report');
        if (request()->has('startdate')) {

            $start_date = request('startdate');
            if ($start_date != "") {
                $query->whereDate('carddaily_report.report_date', '>=', date('Y-m-d', strtotime($start_date)));
            }
            ...
            if (request()->has('enddate')) {
                ...
                $end_date = request('enddate');
                if ($end_date == $start_date) {
                    $query->whereDate('carddaily_report.report_date', '>=', date('Y-m-d', strtotime($end_date)));
                }
            }
        }
        if (request()->has('enddate')) {
        ...
        if (!empty($titles)) {
            ...
            foreach ($titles as $title) {
                ...
                $nestedData['id'] = $count;
                $nestedData['report_date'] = date('M d, Y h:i A',strtotime($title->report_date));
    ...
    public function updatereport()
    {
        ...
        $datee = date('Y-m-d');
        $sponserdate = date('Y-m-d', strtotime($datee . ' -36 day'));
        $query = DB::table('cardchallenges')
            ->whereIn('status' ,['winnerdeclared','lefted'])
            ->whereDate('created_at', '>=', $sponserdate)
            ->get();
        $admin_amt_received = 0;
        $bonus = 0;
        if (!empty($query->toArray())) {
            foreach ($query as $value) {
                $admin_amt_received = 0;
                $data = array();
                $matchkey = $data['matchkey'] = $value->id;
                // $data['name'] = $value->name;
                $data['start_date'] = $value->created_at;
                # recorrect
                $real_money = DB::table('leaugestransactions')
                    ->where('challengeid', $value->id)
                    ->where('matchkey', 0)
                    ->select(
                        DB::raw('sum(balance) as balance'),
                        DB::raw('sum(winning) as winning'),
                        DB::raw('sum(bonus) as bonus'),
                        DB::raw('sum(extracash) as extracash')
                    )
                    ->first();
                ...
                # Admin Amt Received
                $actual_received = ($real_money->winning + $real_money->balance + $real_money->extracash);

                $admin_amt_received += $actual_received;

                $data['invested_amt'] = number_format($admin_amt_received, 2, '.', '');

                $winning_amt = DB::table('cardfinalresults')->where('challengeid',$value->id)->where('matchkey', 0)->sum('amount');
                ...
                $refund_amt = DB::table('refunds')->where('challengeid',$value->id)->where('matchkey', 0)->sum('amount');
                ...
                $youtuber_bonus = DB::table('youtuber_bonus')->where('challengeid',$value->id)->where('matchkey', 0)->sum('amount');
                ...
                $data['amount_profit_or_loss'] = number_format($amount_profit_or_loss, 2, '.', '');
                $val = DB::table('cardprofitloss')->where('matchkey', $value->id)->first();
                if (empty($val)) {
                    DB::connection('mysql2')->table('cardprofitloss')->insert($data);
                } else {
                    DB::connection('mysql2')->table('cardprofitloss')->where('matchkey', $value->id)->update($data);
                }
    ...
    public function view_profit_loss_dt(Request $request)
    {
        ...
        $query = DB::table('cardprofitloss')
            ->orderBy('cardprofitloss.start_date', 'desc');

        // search for the series name //
        // if (isset($_GET['name'])) {
        //     $name = $data['name'];
        //     if ($name != "") {
        //         $query->where('cardprofitloss.name', 'LIKE', '%' . $name . '%');
        //     }
        // }
        ...
        if (!empty($titles)) {
            ...
            foreach ($titles as $title) {
                ...
                $nestedData['start_date'] = date('M d, Y h:i A', strtotime($title->start_date));
