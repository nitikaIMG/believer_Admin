			...
			if(!empty($aaa)){
				foreach($matchplayers as $row){
					...
					if($match == 't20'){
						...
						if($findplayerrole->role!='bowler'){
							if($bballs>=5){
								...
								else if($strikerate>=60 && $strikerate<=69.9){
								$strikePoint = -4;
								}
						...
						if($overs>=2){
							...
							else if($erate>=4 && $erate<=4.99){
								$economypoints = 4;
							}
							else if($erate>=5 && $erate<=6){
								$economypoints = +2;
							}
							else if($erate>=9 && $erate<=10){
								$economypoints = -2;
							}
							else if($erate>=10.1 && $erate<=11){
								$economypoints = -4;
							}
							else if($erate>=11){
								$economypoints = -6;
						...
						if($bballs>=20){
							...
							else if($strikerate>=40 && $strikerate<=49.99){
								$strikePoint = -4;
							}
					...
					if($row->starting11==1){
						...
						// $result['batting_points'] = $runpoints+$sixpoints+$boundrypoints+$strikePoint+$halcenturypoints+$centuryPoints;
						...
						// $total_points = $result['total_points'] = $startingpoint+$runpoints+$sixpoints+$halcenturypoints+$centuryPoints+$boundrypoints+$strikePoint+$catchpoint+$stpoint+$wkpoints+$maidenpoints+$economypoints+$duckpoint+$hittspoints+$throwpoint+$extra_points;
						//==================================================================
						...
						$total_points = $result['total_points'] = $startingpoint+$runpoints+$sixpoints+$halcenturypoints+$centuryPoints+$boundrypoints+$strikePoint+$catchpoint+$stpoint+$wkpoints+$maidenpoints+$economypoints+$duckpoint+$hittspoints+$throwpoint+$extra_points+$thirtypoints+$wicketbonuspoint;
					}
					else{
               ...
               if(empty($checkpre)){
                   ...
                   if(!empty($giveresresult['data']['card']['innings'])){
                       ...
                       if(!empty($giveresresult['data']['card']['innings']['b_2'])){
							...
							if($findmatchtype->format=='100-ball'){
									$gettestover1 = $giveresresult['data']['card']['innings']['a_2']['balls'];
									$gettestover2 = $giveresresult['data']['card']['innings']['b_2']['balls'];
							}else{
                   ...
                   if(empty($gettestover1)){
                        $matchdata1['overs1'] = ($findmatchtype->format=='100-ball')?$giveresresult['data']['card']['innings']['a_1']['balls']:$giveresresult['data']['card']['innings']['a_1']['overs'];
                   }else{
                       $matchdata1['overs1'] = $giveresresult['data']['card']['innings']['a_1']['overs'].','.$gettestover1;
                   }
                   if(empty($gettestover2)){
                        $matchdata1['overs2'] = ($findmatchtype->format=='100-ball')?$giveresresult['data']['card']['innings']['b_1']['balls']:$giveresresult['data']['card']['innings']['b_1']['overs'];
                   }else{
									...
									// batting points //
										...
										if(isset($batting['ball_of_dismissed'])){
											...
											if($batting['ball_of_dismissed']['wicket_type']=='lbw' || $batting['ball_of_dismissed']['wicket_type']=='bowled'){
												...
												$bowlerplayersid =DB::table('matchplayers')
	 ...
	 if(!empty($abcdd)){
		...
		if(!empty($allchallenges)){ 
			foreach($allchallenges as $challenge){
				 ...
				 if($challenge->maximum_user > $challenge->joinedusers){
    				...
    				// ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
    				elseif($challenge->confirmed_challenge == 1 && $challenge->joinedusers==1){
    				    $getresponse = $this->refundprocess($challenge->id,$challenge->entryfee,$matchkey,'challenge cancel');
    				    if($getresponse==true){
    				        $data['status'] = 'canceled';
    				        DB::connection('mysql2')->table('matchchallenges')->where('id',$challenge->id)->update($data);
