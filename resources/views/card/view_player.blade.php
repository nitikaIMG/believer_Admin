    ...
    <div class="col-md-12">
      <div class="card mb-3">
        <div class="card-heading p-3">
          <form method="get" action="<?php echo action('CardController@view_player')?>">
                  <?php
                  $team="";$playername="";$role="";;
                  if(isset($_GET['team'])){
                    $team = $_GET['team'];
                  }
                        ...
                        <div class="row mx-0">
                            ...
                            <div class="col-12 text-right mt-4 mb-2">
                              <button class="btn btn-sm btn-success text-uppercase"><i class="far fa-check-circle"></i>&nbsp;Submit</button>
                                <a href="{{action('CardController@view_player')}}" class="btn btn-sm btn-warning text-uppercase"><i class="far fa-undo" ></i>&nbsp; Reset</a>
                            </div>
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    $.fn.dataTable.ext.errMode = 'none';
        $('#view_player_datatable').DataTable({
            ...
            "serverSide": true,
            "ajax":{
                     "url": '<?php echo URL::asset('my-admin/view_cardplayer_datatable');?>?playername='+playername+'&team='+team+'&role='+role,
                     "dataType": "json",
                     "type": "POST",
...
var credit= $("#getcredittd"+id).val();
  if(credit>15){
    ...
    $.ajax({
      type:'POST',
      url:'<?php echo asset('my-admin/savecardplayerroles');?>',
...
function get_teams(){
    $.ajax({
    type:'POST',
    url:'<?php echo asset('my-admin/get_cardteams');?>',
